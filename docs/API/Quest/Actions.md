...
public class MyBanActionFactory implements PlayerActionFactory {
  ...
  public PlayerAction parsePlayer(final Instruction instruction) throws QuestException {
    final String message = instruction.string().get().getValue(null); // get the message from the instruction
...
Having the message configurable is a good, but using the `Argument` container is even better.
In the third approach, we passed the message to the action as an `Argument` and the `Instruction` is designed to make it as easy as possible to create an `Argument` for a specific value.
...
public class MyBanActionFactory implements PlayerActionFactory {
  ...
  @Override
  public PlayerAction parsePlayer(final Instruction instruction) throws QuestException {
    final Argument<String> message = instruction.string().get(); // get the message Argument from the instruction
    return new OnlineActionAdapter(new MyBanAction(message));
  }
...
Since we are no longer using an `OnlineAction` but a `PlayerlessAction` instead, we may also drop the `OnlineActionAdapter` and just return the action directly.
...
public class MyBanActionFactory implements PlayerlessActionFactory {
  ...
  public PlayerlessAction parsePlayerless(final Instruction instruction) throws QuestException {
    final Argument<UUID> playerUID = instruction.uuid().get(); // get the player UID Argument from the instruction
    final Argument<String> message = instruction.string().get(); // get the message Argument from the instruction
...
public class MyPlugin extends JavaPlugin {
  ...
  public void onEnable() {
    // ...
    betonQuestApi.actions().registry().register("ban", new MyBanActionFactory()); //Approach #1-4
    // ...
  }
...
public class MyPlugin extends JavaPlugin {
  ...
  public void onEnable() {
    // ...
    betonQuestApi.actions().registry().registerCombined("ban", new MyBanActionFactory()); //Approach #5
    // ...
  }
...
public class MyFeature {
  ...
  public void foo() {
    ...
    betonQuestApi.actions().manager().run(profile, action);
  }
}
```
...
public class MyFeature {
  ...
  public void foo() {
    final ActionIdentifier action;
    betonQuestApi.actions().manager().run(null, action);
  }
}
```
...
public class MyFeature {
  ...
  public void foo() {
    final Set<ActionIdentifier> actions; //any java.util.Collection is supported
    betonQuestApi.actions().manager().run(null, actions);
  }
}
```
    ...
    public class MyBanAction implements NullableAction {
      ...
      public void execute(@Nullable Profile profile) throws QuestException {
        ...
        if(this.duration != null) {
          ...
          final Date expirationDate = new Date(System.currentTimeMillis() + expirationMillis);
...
??? example "MyBanActionFactory.java"
    ...
    public class MyBanActionFactory implements PlayerActionFactory, PlayerlessActionFactory {
      ...
      public NullableActionAdapter parse(final Instruction instruction) throws QuestException {
        final Argument<UUID> playerUID = instruction.uuid().get();
        final Argument<String> message = instruction.string().get();
        final Argument<TimeUnit> unit = instruction.enumeration(TimeUnit.class).get("unit", TimeUnit.DAYS);
        final Optional<Argument<Number>> duration = instruction.number().get("time");
